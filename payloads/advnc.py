...
def http(ipaddr):
    ...
    if not s.connect_ex((ipaddr,80)):
        ...
        s.close()
 # https --- tries to connect to ssl webservers
def https(ipaddr):
    s = socket(AF_INET, SOCK_STREAM)
    s.settimeout(0.3)
    if not s.connect_ex((ipaddr,443)):
        s.close()                      
        return 1
    else:
        s.close()
...
def printSuccess(text):
	print(colors.fg.lightgreen+"✔  "+colors.reset+text)
def printInfo(text):
	print("\033[96mi  "+colors.reset+text)
...
if __name__ == '__main__':
	...
	clear();
	# banner();
	printInfo("Collecting Local Information")
	ip = get_lan_ip()
	printInfo("Local IP is \033[96m"+ip+colors.reset)
	ipTemp = ip.split(".")
	subnet = ""
	i = 0
	for x in ipTemp:
		if i < 3:
			subnet = subnet + x + "."
		i=i+1
	# printInfo("Subnet is \033[96m" + subnet + colors.reset)
	printSuccess("Finished Collecting Info")
	time.sleep(2)
	clear()
	print(colors.fg.lightcyan+"\n[1] Single IP Scan\n")
	print(colors.fg.cyan+"[2] Automated Scan"+colors.reset)
	which = input("\033[96mi  "+colors.reset+"Choice: ")
	if which == "1":
		from socket import *
		time.sleep(1.5)
		ipaddr = input("\n\033[96mi  "+colors.reset+"Target: ")
		if ping(ipaddr):
			printSuccess(colors.fg.lightgreen+ipaddr+colors.reset+" is online")
			if http(ipaddr):
				printInfo("⟼  Computer running HTTP ("+colors.fg.lightgreen+"80"+colors.reset+")")
			if https(ipaddr):
				printInfo("⟼  Computer running HTTPS ("+colors.fg.lightred+"443"+colors.reset+")")
			if ftp(ipaddr):
				printInfo("⟼  Computer running FTP ("+colors.fg.blue+"21"+colors.reset+")")
			for val, i in enumerate(common_port_list):
				...
				if (result == 0):
					printInfo("⟼   "+service(i)+" ("+str(i)+") is open ")
					
		else:
			print("Host not online. Exiting..")
			sys.exit(0)
	else:
		printInfo("Starting Host Discovery")
		from socket import *
		for ip in range(1,254):
			ipaddr = subnet + str(ip)
			if ping(ipaddr):
				printSuccess(colors.fg.lightgreen+ipaddr+colors.reset+" is online")
				if http(ipaddr):
					printInfo("⟼  Computer running HTTP ("+colors.fg.lightgreen+"80"+colors.reset+")")
				...
				if ftp(ipaddr):
					printInfo("⟼  Computer running FTP ("+colors.fg.blue+"21"+colors.reset+")")
				printSuccess("Finished Common Service scan")
				time.sleep(0.5)
				printInfo("Starting Port Scan/Service Scan")
				for val, i in enumerate(common_port_list):
					ssk = socket(AF_INET, SOCK_STREAM)
